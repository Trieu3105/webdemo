...
export default function Dangnhatl() {
  const [giftsLogin, setGiftsLogin] = useState<giftsplay[]>([]);
    ...
    async function fetchGiftsLogin() {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/giftplay"
        );
        // Loại bỏ trùng lặp trong gift_name dựa trên id
        const filteredData = response.data.map((gift: giftsplay) => ({
          ...gift,
              ...
              {giftsLogin.map((gift) => (
                <tr key={gift.id}>
                  <td className="whitespace-nowrap px-3 py-4 text-gray-500 dark:text-gray-400 text-sm">
                    <p className="break-words font-semibold">{gift.house} Giờ</p>
                  </td>
                  <td className="whitespace-nowrap px-3 py-4 text-gray-500 dark:text-gray-400 text-sm">
